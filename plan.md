...
- WS-21: merged (2026-04-21, commit `44b137f`). `bm-pilot backfill [--since 30d|7d|72h|all] [--dry-run] [--force] [--adapter claude-code|codex] [--json]`. Three new readers: `apps/ingest/src/adapters/claude-code/backfillReader.ts` + codex counterpart expose `enumerateHistoricalFiles({root, sinceMs})` (mtime-filtered file list) and `readFileToEnvelopes(path, ctx)` (async-iterable envelope stream; emits synthetic `session_start` first, line-derived envelopes, then `session_end`). Claude Code reuses `parseLineToEnvelopes` + `makeSessionStart/EndEnvelope`; codex reuses `parseRolloutLine` + a fresh per-file `TokenDiffer` so cumulative `token_count` records produce per-turn deltas identical to the live tailer. Codex honors rollout's own `session_end` if present; otherwise synthesizes `reason:"backfill"`. Orchestrator in `apps/ingest/src/commands/backfill.ts` batches at size 100, drains through the real `Uploader` (same auth/retry/dedup as the daemon), and on success writes EOF to `~/.bm-pilot/offsets-claude-code.json` (`{version:1,files:{}}`) + `~/.bm-pilot/offsets-codex.json` (`{codex:{}}`) via atomic tmp+rename. Codex writes sit under a local `~/.bm-pilot/offsets-codex.lock` reimplementing the tailer's mtime-stale protocol (existing `withLock` is not exported; dedup is a future cleanup if needed). `UploadRetriesExhaustedError` on any batch fails the adapter run, leaves offsets untouched, exits nonzero. Refuses without valid ingest key; refuses when `~/.bm-pilot/daemon.pid` points to a live process (checked via `process.kill(pid,0)`) unless `--force`. Cursor always reports `skipped (no historical record)`. Client emits a fresh `randomUUID()` session_id per file; idempotency relies on WS-2's `sessions (source, source_session_id)` upsert — this was verified as the correct dedup key, not `session_id`. 22 new tests (6 claude-code reader + 6 codex reader + 10 orchestrator); full suite 482/482.
- WS-22: merged (2026-04-21, commit `38955af`). Auto-prompt backfill in `bm-pilot start`. `apps/ingest/src/commands/backfillConsent.ts` exports `ensureBackfillConsent(deps)` with full injectable seams (`prompts`, `detectHistorical`, `runBackfill`, `now`, `out`, `isTty`, `flag`, `configPath`) and a discriminated-union `ConsentResult`. State machine: already-asked short-circuits; zero-history silently persists declined; non-interactive w/o flag persists declined; prompt shown only when history ≥ 1 AND stdin is a TTY AND no flag; default is N (enter=N); `--backfill`/`--no-backfill` bypass prompt and persist directly. Critically, config is persisted with `backfillAccepted: true` BEFORE `runBackfill` is awaited so a throwing uploader does not leave the dev in a re-prompt loop (they retry via explicit `bm-pilot backfill`). ConfigSchema gains strict optional `onboarding: { backfillPromptedAt?, backfillAccepted? }`; `freshConfig()` leaves it undefined. `onboard.ts` exports `runBackfillConsentStep(opts)` that wires production deps — calls WS-21's already-exported `runBackfill(opts)` (no restructuring of `backfill.ts` needed) with `force: true` (service is running by this point, and server dedup covers it), `since: "30d"`, `dryRun: false`. Integrated in both the normal and fallback-daemon paths of `cmdStart`. `parseStartFlags` handles `--backfill` / `--no-backfill` on the `start` subcommand. Output on run: `[ok]  backfill: N files · M envelopes · K KB uploaded`; on decline: `[skip] backfill declined`; non-interactive: `[skip] backfill (non-interactive)`. 15 new tests (11 consent state machine + 4 onboard wiring); full suite 497/497.
