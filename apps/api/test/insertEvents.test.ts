...
describe("insertEvents", () => {
  it("inserts a batch and reports accepted count", async () => {
    ...
    const events = [
      makeEnvelope({ session_id: "s1", event_seq: 0 }),
      makeEnvelope({ session_id: "s1", event_seq: 1 }),
      makeEnvelope({ session_id: "s1", event_seq: 2 }),
    ];
  ...
  it("dedups on (org_id, session_id, event_seq, client_event_id)", async () => {
    const seeded = await seedIngestKey(sql);
    const event = makeEnvelope({ session_id: "dedup-session", event_seq: 7 });
  ...
  it("inserts two events with different event_seq values", async () => {
    ...
    const a = makeEnvelope({ session_id: sameSession, event_seq: 0 });
    const b = makeEnvelope({ session_id: sameSession, event_seq: 1 });
  ...
  it("computes cost_usd inline from pricing table", async () => {
    ...
    const rows = await sql.begin(async (tx) => {
      ...
      return tx<{ cost_usd: string; pricing_version: string }[]>`
        SELECT cost_usd::text, pricing_version
        FROM events
        WHERE session_id = 'cost-s' AND event_seq = 0 AND org_id = ${seeded.orgId}
      `;
    });
    expect(rows.length).toBe(1);
    const row = rows[0]!;
    expect(Number(row.cost_usd)).toBeCloseTo(3.0 + 7.5, 6);
    expect(row.pricing_version).toBe("test-v1");
  });

  it("persists developer_id from the auth seam, ignoring envelope inputs", async () => {
    const seeded = await seedIngestKey(sql);
    const event = makeEnvelope({ session_id: "owner-check", event_seq: 0 });
    await insertEvents(sql, seeded.orgId, seeded.developerId, [event]);
    const rows = await sql.begin(async (tx) => {
      await tx`SELECT set_config('app.current_org_id', ${seeded.orgId}, true)`;
      return tx<{ developer_id: string; org_id: string }[]>`
        SELECT developer_id, org_id FROM events
        WHERE session_id = 'owner-check' AND org_id = ${seeded.orgId}
      `;
    });
  ...
  it("enforces RLS: cross-org reads return 0 rows", async () => {
    ...
    const eventA = makeEnvelope({ session_id: "rls-a", event_seq: 0 });
    ...
    const visibleToB = await sql.begin(async (tx) => {
      ...
      return tx<{ n: string }[]>`
        SELECT count(*)::text AS n FROM events WHERE session_id = 'rls-a'
      `;
    });
